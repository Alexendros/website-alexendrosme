...
test.describe('Landing · smoke + anchors + FABs', () => {
  ...
  test('skip-link es el primer elemento enfocable y apunta a #main', async ({
    page,
  }) => {
  ...
  test('nav contiene los 3 anchors correctos', async ({ page, viewport }) => {
    ...
    if (isMobile) {
      ...
      await expect(
        page.locator("a[href='#experiencias']").first(),
      ).toBeAttached()
    } else {
  ...
  test('nav desktop: enlace Productos visible con href/target/rel correctos', async ({
    ...
    const isMobile = !viewport || viewport.width < 768
    if (!isMobile) {
      const productosLink = page
        .locator("header nav a[href='https://alexendros.dev']")
        .first()
  ...
  test('nav mobile: enlace Productos NO visible (solo desktop)', async ({
    page,
    viewport,
  }) => {
    const isMobile = !viewport || viewport.width < 768
  ...
  test('FABs: Escape cierra el popover y devuelve foco al trigger', async ({
    page,
  }) => {
    const fab = page.locator('button', { hasText: 'Convócame' })
    await fab.click()
  ...
  test('footer: enlace Hub de productos visible con href/target/rel correctos', async ({
    ...
    const footerLink = page
      .locator("footer a[href='https://alexendros.dev']")
      .first()
