...
test.describe('Anti-monetization banner', () => {
  ...
  test('visible on first visit with correct text and icon', async ({
    page,
  }) => {
    await page.goto('/')
    ...
    // Check text content
    await expect(
      banner.locator('.anti-monetization-banner__text'),
    ).toContainText(
  ...
  test('banner does not intercept clicks on elements below it after dismiss', async ({
    ...
    test.skip(
      ...
      'Mobile nav is in Sheet, not directly clickable',
    )
    await page.goto('/')
    await page.waitForLoadState('networkidle')
