...
describe("getRawContent", () => {
  ...
  it("caches article data with content", async () => {
    ...
    /* eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- safe after not.toBeNull() assert */
...
describe("getRelatedContent", () => {
  ...
  it("excludes the current article from results", async () => {
    ...
    const target = items[0] as NonNullable<(typeof items)[number]>;
